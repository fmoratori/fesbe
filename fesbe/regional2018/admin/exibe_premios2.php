...
$cate = '';
if($categ==1){
$cate = "AND usuario_id IN (SELECT user_id FROM `tb_boleto` WHERE `categoria`=1 OR `categoria`=2)";
}
if($categ==2){
$cate = "AND usuario_id IN (SELECT user_id FROM `tb_boleto` WHERE `categoria`=5 OR `categoria`=6)";
}
if($categ==3){
$cate = "AND usuario_id IN (SELECT user_id FROM `tb_boleto` WHERE `categoria`!=1 AND `categoria`!=2 AND `categoria`!=5 AND `categoria`!=6)";
}
...
<div class="ls-box">
  <main class="ls-main-full">
      <div>
<?
$sql30 = "SELECT * FROM `tb_trabalhos` WHERE `premio`=$premio AND `status`=4 $cate ORDER BY `id` ASC";
...
echo     "<h3 class='ls-collapse-title'><center>ID ".$id_trabalho."<br />".utf8_encode($row30['titulo'])."</center></h3></a><div class='ls-collapse-body' id='$id_trabalho'>";
