...
<h1 class="ls-title-intro ls-ico-eye">Painel de Avaliação</h1>
	...
	$id_aval = $row222['id'];
		$count10 = mysql_query("SELECT COUNT(*) FROM `tb_trabalhos` WHERE avaliador_painel = $id_aval");
	...
	$res223 = mysqlexecuta($id,$sql223);
		while($row223 = mysql_fetch_array($res223)){
			$status = $row223['id'];
			$count9 = mysql_query("SELECT COUNT(*) FROM `tb_trabalhos` WHERE avaliador_painel = $id_aval AND status = $status");
