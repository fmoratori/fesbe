...
<?
	...
	echo "<tr><td colspan='2'>".utf8_encode('<b>TOTAL NÃO PAGO</b>')."</td><td><b>".$status4."</b></td>"."<td>".$porc_pg_np."%</td></tr>";		
	...
	$count5b = mysql_query("SELECT SUM(valor) FROM `tb_boleto` WHERE situacao = 'PG' OR situacao='IS'");
...
<div class="ls-box">
  ...
  <h5 class="ls-title-3">Trabalhos Por Status</h5>
...
<div class="ls-box">
  ...
  <h5 class="ls-title-3">Trabalhos Por &Aacute;rea - APROVADOS</h5>
...
<?
	...
	$sql10 = "SELECT * FROM tb_areas ORDER BY id";
	...
	echo "<tr><td colspan='2'>".$row10['id']." - ".utf8_encode($row10['nome_area'])."</td><td>".$area."</td>"."</tr>";
...
<div class="ls-box">
  ...
  <h5 class="ls-title-3">Trabalhos Por Estado - NOVO</h5>
...
<?
	...
	$sql101 = "SELECT DISTINCT estado,COUNT(*) AS quantidade FROM tb_usuario WHERE id IN (SELECT usuario_id FROM tb_trabalhos WHERE `status`=4) GROUP BY estado ORDER BY quantidade DESC";
...
<?
	...
	$sql115 = "SELECT * FROM tb_atividades";
	...
	$count6 = mysql_query("SELECT COUNT(*) FROM `tb_usuario` WHERE inscricao_obs = $f OR inscricao_obs2 = $f OR inscricao_obs3 = $f OR inscricao_obs4 = $f");
