...
if($sessao_id=='0'){
    ?>
        <p><div class="ls-alert-danger">N&atilde;o Existem Trabalhos Para Serem Avaliados no dia de hoje.</div></p>
<?
exit();
}     
$sql2 = "SELECT * FROM `tb_trabalhos` WHERE avaliador_painel = '$id_avaliador' AND sessao_id='$sessao_id' AND `nota_apresentacao` < 1 ORDER BY area_id ASC";
...
<p><h2><center>LISTA DE TRABALHOS NÃO AVALIADOS</center></h2></p>
<?
}
...
$sql53 = "SELECT * FROM `tb_trabalhos` WHERE avaliador_painel = '$id_avaliador' AND sessao_id='$sessao_id' AND (mencao='s' OR mencao='n') ORDER BY area_id ASC";
$res53 = mysqlexecuta($id,$sql53); 
$row53 = mysql_fetch_array($res53);
if($row53['id']==''){

if($x==1){
$sql52 = "SELECT * FROM `tb_trabalhos` WHERE avaliador_painel = '$id_avaliador' AND sessao_id='$sessao_id' ORDER BY area_id ASC";
...
if($qp==1){
?>
    <p><h2><center>LISTA DE TRABALHOS JÁ AVALIADOS</center></h2></p>
<?
}
$descricao = $row2['titulo'];
$descricao = substr($descricao, 0, 50);
