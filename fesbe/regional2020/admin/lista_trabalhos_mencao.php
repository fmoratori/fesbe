...
$sql = "SELECT * FROM `tb_trabalhos` WHERE area_id=$area AND status LIKE '4' AND mencao='s' ORDER BY painel ASC";
...
$row3 = mysql_fetch_array($res3);
	...
	$count = mysql_query("SELECT COUNT(*) FROM `tb_trabalhos` WHERE area_id=$area AND status LIKE '4' AND mencao='s' ORDER BY painel ASC");
...
<p>
   ...
   <!--tr-->
	...
	<!--td--><? echo $area_id.".".$row['painel']." - <span class='uppercase'>".trim($titulo)."</span>. <BR />".trim($autores)." - " ?>
