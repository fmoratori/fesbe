...
$autenticacao = sha1($titulo_exibe.'mencao');
    ...
    $texto = "Certificamos que o trabalho ".$titulo_exibe." foi contemplado com MENÇÃO HONROSA durante a ".$row_evento['nome_evento'].", realizado de ".$inicio." a ".$fim." no ".$row_evento['local'].".";
	$pdf= new FPDF("L","pt","A4");
	$pdf->AddPage();
	$pdf->Image("./cabecalho.jpg",0,0,842,140);
	$pdf->Ln(150);
	$pdf->SetFont('arial','B',24);
	$pdf->Cell(0,20,utf8_decode("MENÇÃO HONROSA"),0,1,'C');
	$pdf->Ln(30);
...
//	$pdf->Image("./assinaturas.jpg",210,430,420,90);
	...
	$pdf->Image("./rodape.jpg",10,485,820,100);
  //  ob_start();
 	$pdf->Output($autenticacao.".pdf","I");
 	$pdf->Output("./certificados_mencao/".$autenticacao.".pdf","F");
?>