...
if($status==3){
    $sql_finaliza_envio="UPDATE `tb_trabalhos` SET `status`='$status' WHERE `id`=$id_trabalho;";	
 ...
 $aval = $row_usuario['avaliador_id'];
         $titulo = $row_usuario['titulo'];
 
 /** Status de Corrigido + E-mail para o avaliador **/   
    if($row_usuario['parecer']!=''){
          $sql_finaliza_envio="UPDATE `tb_trabalhos` SET `status`='9' WHERE `id`=$id_trabalho;";	
            $res_finaliza_envio = mysqlexecuta($id,$sql_finaliza_envio);  
            
            
    $sql_aval = "SELECT * FROM tb_avaliador WHERE `id`=$aval";
    $res_aval = mysqlexecuta($id,$sql_aval);
    $row_aval = mysql_fetch_array($res_aval);             
      $aval_email = $row_aval['email'];      
if(PHP_OS == "Linux") $quebra_linha = "\n"; //Se for Linux
elseif(PHP_OS == "WINNT") $quebra_linha = "\r\n"; // Se for Windows
else die("Este script nao esta preparado para funcionar com o sistema operacional de seu servidor");

$mensagemHTML = "O trabalho <b>$titulo</b> acaba de ser corrigido.<br /> Por favor acesse sua &aacute;rea de avalia&ccedil;&atilde;o.<br /><br />Atenciosamente,<br />Comiss&atilde;o Organizadora <br />".$row['nome_evento'];
$mail = new PHPMailer();
$mail->IsSMTP();
$mail->SMTPAuth = true;
$mail->SMTPSecure = "ssl";
...
$mail->Subject = 'Trabalho Corrigido '.$row['nome_evento'];
$mail->MsgHTML($mensagemHTML);
//$mail->AddAttachment($link);
$mail->AddAddress($aval_email);
