...
<div class="ls-box-filter">
  ...
  <form action="admin_avaliacao_painel.php?pg=consulta_trabalhos_avaliacao_painel.php&ver=1" class="ls-form ls-form-inline row" method="POST">
 <fieldset>
...
if($ver==1){
    ...
    $sql_consulta_trabalhos = "SELECT * FROM tb_trabalhos WHERE `usuario_id` $complemento AND `status`=4 ORDER BY `painel` ASC";
...
$x=0;
   while($row_consulta_trabalhos = mysql_fetch_array($res_consulta_trabalhos)){
    ...
    echo "<td>".$row_consulta_area['id'].". ".utf8_encode($row_consulta_area['nome_area']).'</td>';
    ...
    echo "<td>".$row_consulta_trabalhos['painel']." / ".$row_consulta_trabalhos['sessao_id']."</td>";
    $id_status = $row_status_trabalhos['id'];
    $avaliador_id = $row_consulta_trabalhos['avaliador_painel'];
    echo "<td>";
//    echo $row_status_trabalhos['status'];
?>
<?
    $avaliador = $row_consulta_trabalhos['avaliador_painel'];
...
?>      
 ...
 <form action="troca_avaliador_painel.php?ver=1&fonte=2&id_trabalho=<? echo $id_trabalho; ?>" class="ls-form ls-form-inline ls-float-right" method="POST" target="_blank">
...
<? 
    echo "</td>";
      ...
      //  echo "<a href='./principal_admin.php?pg=boletos.php&id_inscrito=".$row_consulta_trabalhos['id']."' class='ls-ico-cart' title='Dados de Boletos'></a></td>";
        echo "<td>";
?>
<a href="#" class='ls-ico-pencil' onclick="window.open('http://www.fmsys.com.br/fmsys/fesbe/regional2019/admin/troca_avaliador.php?id_trabalho=<? echo $id_trabalho; ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=770, HEIGHT=400');"></a>
<?
if($status!=1){
?>
<a href="#" class='ls-ico-docs' onclick="window.open('http://www.fmsys.com.br/fmsys/fesbe/regional2019/user/exibe_trabalho.php?id_trabalho=<? echo $id_trabalho; ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=770, HEIGHT=400');"></a>
<?
}
if($status!=10){
?>
<a href="#" class='ls-ico-user' onclick="window.open('http://www.fmsys.com.br/fmsys/fesbe/regional2019/admin/autores_admin.php?id_trabalho=<? echo $id_trabalho; ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=770, HEIGHT=400');"></a>
<?
}
